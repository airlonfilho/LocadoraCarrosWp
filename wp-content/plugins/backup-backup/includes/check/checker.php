...
class BMI_Checker {
  ...
  public function check_free_space($size) {
    ...
    $this->logs(__('Requires at least ', 'backup-migration') . $size . __(' bytes.', 'backup-migration') . ' [' . BMP::humanSize($size) . ']');
    if ($this->is_enabled('disk_free_space') && intval(disk_free_space(BMI_BACKUPS)) > 100) {
      ...
      $this->logs(__('Checking this path/partition: ', 'backup-migration') . BMI_BACKUPS);
      $free = intval(disk_free_space(BMI_BACKUPS));
      $this->logs(__('There is ', 'backup-migration') . number_format($free / 1024 / 1024, 2) . __(' MB free.', 'backup-migration') . ' [' . BMP::humanSize($free) . ']', 'SUCCESS');
      if ($free > $size) {
