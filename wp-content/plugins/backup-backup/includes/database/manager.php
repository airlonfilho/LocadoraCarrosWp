...
class BMI_Database {
  ...
  public function exportDatabase($backup_name = false) {
    ...
    foreach ($target_tables as $table) {
      ...
      if ($chunks > 0) {
        $g = 0;
        while($g < $chunks) {
          if (($g + 1000) > $chunks) $limit_str = 'LIMIT '. $g . ',10000';
          else $limit_str = 'LIMIT '. $g . ',1000';
  ...
  public function importDatabase($file, $old_domain, $new_domain, $abs, $newabs, $newprefix) {
    ...
    if (mysqli_connect_errno()) {
      Logger::error(__("Failed to connect to MySQL: ", 'backup-migration') . mysqli_connect_error());
      return __("Importing failed - cannot connect to the database.", 'backup-migration');
    }
    ...
    if ($handle) {
      ...
      Logger::error(__("Cannot open SQL file...", 'backup-migration'));
      return __("Importing failed - Cannot open SQL file...", 'backup-migration');
    }
